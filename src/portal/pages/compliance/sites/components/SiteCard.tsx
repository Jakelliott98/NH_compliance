    ...
    return (
        <div className={`${ isCalibrated ? 'bg-l-success' : 'border-l-warning'} bg-white rounded w-full h-full p-4 border-l border-l-5 border-l-green-700 border border-gray-400 cursor-pointer`}>
            <div className="border-b-1 border-gray-200 flex flex-col pb-1">
                    <p className="text-sm text-neutral-light">{site.site_name}</p>
                    <p className="text-xs text-neutral-light">{site.team_leader}</p>
            </div>
            ...
            <div className="pt-2 sm-hidden-block">
                {
                    sortedAfinions.length === 0 && (
                        <p className='text-xs italic text-neutral-light'>No afinions added yet</p>
                    )
                }
                {
                    sortedAfinions.map((afinion) => {
                        return (
                            <div className="flex justify-between" key={afinion.afinion_id}>
                                <p className='text-xs text-neutral-light'>Afinion {afinion.afinion_number}</p>
                                <p className={`text-xs ${!afinion.last_calibrated && 'italic'} text-neutral-light`} >{afinion.last_calibrated ? moment(afinion.last_calibrated).format('DD MMM') : 'No calibrations'}</p>
                            </div>
                        )
                    })
                }
